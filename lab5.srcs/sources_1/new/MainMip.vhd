...
architecture Behavioral of main_mip is
...
MPG1: MPG port map(input => btn(0), clk => clk, en => en);
MPG2: MPG port map(input => btn(1), clk => clk, en => rst); 
...
IDecode: InstructionDecode port map(clk => clk,
                     RegWrite => RegWrite,
                     en => en,
                     RegDst => RegDst,
                     ExtOp => ExtOp,
...
MC: MainControl port map(input => Instruction(15 downto 13), 
                         RegDst => RegDst,
                         ExtOp => ExtOp,
                         ALUSrc => ALUSrc,
                         Branch => Branch,
...
ExUnit: ExecutionUnit port map(RD1 => RD1,
                    RD2 => RD2,
                    Ext_Imm => Ext_imm,
                    func => func,
                    PC_1 => pc_out,
...
Memorie: memory port map(clk => clk,
                     en => en,
                     MemWrite =>  MemWrite,
                     ALUResIn => ALURes,
                     WriteData => RD2,
...
jumpAddr <= pc_out(15 downto 13) & Instruction(12 downto 0);
    
       
--selectia valorii de afisat
process(Instruction,pc_out,RD1,RD2,WD,Ext_Imm,sw)
begin
